...
import { useWinSize, fetchFromStudy, fetchFromSeries } from '../utils/useSelect';
...
const AllPatients: React.FC<AllPatientsProps> = (props) => {
  ...
  const SeriesDateList: any[] = seriesList.filter((array) =>
    array.MainDicomTags.SeriesDate ? array.MainDicomTags.SeriesDate.match(searchValue) : null,
  );
  ...
  function checkList(selectKey: string) {
    ...
    if (selectKey === 'SeriesDate') {
      return fetchFromSeries(SeriesDateList, _SeriesDateList, _StudyDateList, studiesList, patientList);
    }
    ...
    <>
      <div className="global-search-wrapper" style={{ width: winWidth.width }}>
        <Select
          ...
          size="middle"
          style={{ width: 150, display: 'inline-block', marginRight: 8, marginBottom: 8 }}
